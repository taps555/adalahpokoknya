...
router.put('/rab-items/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const {
      name, paymentUnit, category, reference, overhead,
      volume, rabUnitPrice, groupId, components,
    } = req.body;
    ...
    let componentUpdate;
    const overheadPct = overhead != null ? Number(overhead) : (existing.overhead != null ? Number(existing.overhead) : 0.1);
    ...
    const rabPrice = rabUnitPrice != null ? Number(rabUnitPrice) : Number(existing.rabUnitPrice);
...
router.put('/rab-items/:id/switch-job', async (req, res) => {
  try {
    ...
    if (!newJobTypeId) return res.status(400).json({ error: 'Field "newJobTypeId" wajib diisi.' });
