...
function streamBast2Pdf(complaint, formData, res) {
  ...
  const projectData = [
    ["NO. BAST", ":  " + (formData.bastNumber || "-")],
    ["NAMA PROYEK", ":  " + (complaint.project?.name || "-")],
    ["ALAMAT PROYEK", ":  " + (complaint.project?.location || "-")],
    ["PEMBERI TUGAS", ":  " + (formData.pihakPertamaName || "-")],
    ["KONTRAKTOR", ":  PT. DIVES JAYA PERKASA"],
    ["NO. SPK", ":  " + (formData.spkNumber || "-")],
  ];
  ...
  // ==========================================
  y += 15;
  const formattedDate = formatDateId(formData.handoverDate);
  const leftMargin = 35;
  ...
  doc.text(`1.   ${formData.pihakPertamaName || "-"}`, leftMargin, y);
  doc.text(`:   Sebagai pihak pertama ( Pemberi Tugas )`, leftMargin + 160, y);
  y += 12;
  doc.text(`2.   ${formData.pihakKeduaName || "-"}`, leftMargin, y);
  doc.text(
  ...
  y = doc.y + 5;
  const statusText = (formData.statusText || "-").replace(/\r/g, "");
  doc
    .font("Times-Roman")
  ...
  (complaint.categories || []).forEach((cat) => {
    (cat.items || []).forEach((item) => {
      (item.photos || []).forEach((p) => {
        if (p.type && p.type.toUpperCase() === "AFTER") {
          photos.push(p);
        }
      });
    });
  ...
  doc.text(formData.pihakKeduaName || "-", leftMargin, y);
  doc.text(formData.pihakPertamaName || "-", leftMargin, y, {
    align: "right",
    width: contentW,
  });
